...
function opponentNoteHit(id, direction, type, isSustainNote)
 if isStando == false and isOra == false and prepareOra == false and mattAttack == false then 
  for i=0,#dirAnim do
    if direction == i then
...
function onEvent(name, value1, value2)
   ...
   if name == 'attackMadt' then
     mattAttack = true	 
	 doTweenX('mattprepare', 'dad', getProperty('dad.x')- 320, 0.5, 'cubeInOut')  
   end
end
function onStepHit()
  ...
  if isStando == true and mustHitSection == false and curStep % 4 == 0 then
    playAnim('dad', 'stand', false)  
  end    
  if isOra == true then
...
function onTweenCompleted(tag)
   ...
   if tag == 'mattprepare' then
     triggerEvent('Play Animation', 'attack', 'dad')     
	 doTweenX('mattATTACKK', 'dad', getProperty('dad.x')+ 320, 0.25, 'cubeInOut')     
   end
   if tag == 'mattATTACKK' then
     if dodging == false and mattAttack == true then
	    healthPie = healthPie - 1337420
	 end
	 if dodging == true and mattAttack == true then
	    mattAttack = false
        triggerEvent('Play Animation', 'idle', 'dad')  	
	 end
   end
